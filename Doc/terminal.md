...
cd =	{para moverce entre carpetas
...
mkdir = {+nombre hacer directorios(carpeta)}

touch = {+nombre Crear archivos}

rm =	{+nombre borra el archivo
	[-r] para borrar carprtas}

chmod = {cambiar permisos r=4,w=2,x=1
	[777] liberar todos los permisos}

whoami = {nombre del propietario}

su =	{cambiar a usuario root} 

cat = 	{+nombre lee el archivo}

mv =	{+nombre +ruta mueve el archivo
	+nombre +nombre cambia el nombre}

cp =	{+nombre +ruta lo copia en la ruta nueva
	[-r] copia con carpeta}

clear = {limpia la pantalla}

setterm = {[-foreground +color] cambia el color de la 	      letras.
	[-background +color] cambia el fondo }

figlet = {[-f] +tipo +nombre da un tipo de letra
	termux-open http://patorjk.com/software/taag l	      ink de ayuda para probar los tipos de letra}
 
ctrl+alt+c = abrir nueva ventana

ctrl+r = buscador de de comandos ejetudatos usados


Porque el conocimiento es libre, venga !!! vamos a compartirlo.
